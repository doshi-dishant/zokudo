...
public class RetailerPage extends PageBase {

	private WebDriver driver;

	private final static Logger log = LoggerHelper.getLogger(RetailerPage.class);
	...
	private By msgAddRetailerSuccess = By
			.xpath("//img[contains(@src,'source')]//following-sibling::div//*[text()='Agent added successfully']");
	...
	public void searchRetailer(String mobileNumber) {
		clickOnSearchButton();
	...
	public void verifyRetailerAdded(String mobileNumber) {
	...
	public void clickOnSubmitRetailer() {
		try {
	...
	public void verifyAddRetailerSuccessMessage() {
		boolean result = gHelper.IsElementPresentQuick(msgAddRetailerSuccess);
		if (!result)
			log.error(msgAddRetailerSuccess);
		gHelper.reloadPage();
		Assert.assertTrue(result);
	}
}
